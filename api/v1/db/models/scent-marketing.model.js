...
const scentMarketingSchema = new mongoose.Schema(
    {
        course_name: {
            type: String,
            required: true,
        ...
        external_consultants_title: {
            type: String,
            // required: true,
        },
        external_consultants_description: {
            type: String,
            // required: true,
        },
        external_consultants_list: [{
            title: { type: String },
        }],
...
scentMarketingSchema.pre('validate', function (next) {
    ...
    if (this.external_consultants_list && this.external_consultants_list.length > 9) {
    ...
    if (this.syllabus_title_subtitle_section && this.syllabus_title_subtitle_section.length > 5) {
        return next(new Error('syllabus_title_subtitle_section cannot have more than 5 items.'));
    }
...
scentMarketingSchema.pre('findOneAndUpdate', async function (next) {
    ...
    if (update.external_consultants_list && update.external_consultants_list.length > 9) {
        return next(new Error('how_it_different_section cannot have more than 8 items.'));
    }
    ...
    if (update.syllabus_title_subtitle_section && update.syllabus_title_subtitle_section.length > 5) {
        return next(new Error('syllabus_title_subtitle_section cannot have more than 5 items.'));
    }
...
scentMarketingSchema.pre('save', async function (next) {
    if (this.isNew) { 
    const existingRecord = await mongoose.model('scentmarketing').findOne();
    if (existingRecord) {
...
module.exports = mongoose.model("scentmarketing", scentMarketingSchema);
